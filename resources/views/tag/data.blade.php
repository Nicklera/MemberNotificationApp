            ...
            <h3 class="box-title">{{trans_choice('general.tag',2)}}</h3>
             ...
             <div class="box-tools pull-right">
                @if(Sentinel::hasAccess('members.create'))
                    @if(\App\Models\Tag::where('name', 'All Members')->count()!= 0)
                        <a href="{{ url('tag/'.\App\Models\Tag::where('name', 'All Members')->first()->id.'/show') }}"
                    @else
                        <a href="{{ url('tag/1/show') }}"
                    @endif
                       class="btn btn-info btn-sm">{{trans_choice('general.view',1)}} {{trans_choice('general.all',1)}} {{trans_choice('general.member',2)}} {{trans_choice('general.tag',1)}}</a>
                @endif
            </div>
        </div> 
            <div class="box-body">
